 ...
 * @LastEditTime: 2021-08-03 16:11:15
-->
<template>
	<!-- 维护地图容器 -->
	<div id="maintain-map-wrapper" class="w-full h-full" />
	...
	<GridDrawer
		...
		:role="role"
		:map-type="MAP_TYPE.MAINTAIN_MAP"
...
<script>
	...
	export default defineComponent({
		...
		setup() {
			...
			// 渲染地图
			const renderMap = () => {
				map = useMap('maintain-map-wrapper', {
					zoom: 5,
			...
			const addTextMarkers = () => {
				...
				gridList.forEach(grid => {
					...
					if (role === ADMIN_ROLE_TYPE.BD_ADMIN_ROLE && grid.maintainId && grid.centerPoint) {
						const [longitude, latitude] = grid.centerPoint?.split(',')
						textMarkerList.push({
							longitude,
							latitude,
